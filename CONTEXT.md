...
- **Condition key** — coarse sky state derived from a WMO weather code: `clear | cloudy | fog | rain | snow | storm`. Defined in `src/domain/wmo.js`; drives the atmosphere theme, particles, and glyph selection.
- **Atmosphere** — the page background model: gradient stops + aurora blob hues for a condition key and day/night part. Lives in `src/ui/atmosphere.js` (ui may depend on domain; never the reverse).
